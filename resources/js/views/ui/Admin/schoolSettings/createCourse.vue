                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li v-show="institution_type == 'High school'" class="breadcrumb-item active" aria-current="page"><span>subjects</span></li>
                                <li v-show="institution_type == 'Junior high school'" class="breadcrumb-item active" aria-current="page"><span>subjects</span></li>
                                <li v-show="institution_type == 'Primary school'" class="breadcrumb-item active" aria-current="page"><span>subjects</span></li>
                                <li v-show="institution_type == 'Tertiary institution'" class="breadcrumb-item active" aria-current="page"><span>courses</span></li>
                            </ol>
                        </nav>
                    </div>
                </li>
                        ...
                        <div class="table-header">
                            <div class="d-flex align-items-center flex-column flex-sm-row flex-shrink-0">
                                <div class="mb-3 mb-sm-0 order-0 order-sm-1 mr-sm-2">
                                    <b-button variant="success" class="ml-2" v-b-modal.institutionCoursesModalCenter>
                                ...
                                <b-modal id="institutionCoursesModalCenter" title="Create" centered no-fade>
                                    <b-form @submit.prevent="submitInstitutionCourse">
                                        <b-form-row class="mb-4">
                                            <b-form-group label="Course" class="col-md-12">
                                                <b-input type="text" placeholder="e.g Physics, Geography"
                                                    v-model="fields.course">
                                                </b-input>
                                            </b-form-group>

                                            <b-form-group label="Department" class="col-md-12">
                                                <b-select v-model="fields.department" size="sm" class="department"
                                                    id="courseDepartment">
                                                    <b-select-option value="">Choose</b-select-option>
                                                    <b-select-option v-for="department in departments" :key="department.id"
                                    ...
                                    <template #modal-footer>
                                        <b-button variant="default" data-dismiss="modal"
                                            @click="$bvModal.hide('institutionCoursesModalCenter')"><i
                            ...
                            <template #cell(edit)="row">
                                <div class="position-relative">
                                    <router-link style="color: #2196f3;"
                                        :to="`/ui/editAdminInstitutionCourse/` + row.value">
...
export default {
    metaInfo: { title: 'Courses :- Intelligent system' },
    data() {
        return {
            items: [],
            columns: [],
    ...
    created() {
        this.institutionCourse(this.currentPage);
    },

    watch: {
        'fields.department': {
            handler(newDepartment) {
                const selectedDepartment = this.departments.find(department => department.department === newDepartment);
                if (selectedDepartment) {
                    this.fields.department_slug = selectedDepartment.slug; // Bind the department's slug
                }
            },
            immediate: true,
        },
    ...
    methods: {
        ...
        institutionCourse(page) {
            axios
                .get('/api/institutionCourses?page=' + page)
                .then((response) => {
                    this.course = response.data.data;
                    this.currentPage = response.data.current_page;
                    this.lastPage = response.data.last_page;

                    this.items = this.course.map((courseData) => {
                        return {
                            id: courseData.id,
                            course: courseData.course,
                            department: courseData.department,
                            edit: courseData.slug,
        ...
        fetchData(page = 1) {
            axios
                .get(`/api/institutionCourses?page=${page}`)
                .then((response) => {
                    this.items = response.data.data.map((courseData) => {
                        return {
                            id: courseData.id,
                            course: courseData.course,
                            department: courseData.department,
        ...
        async submitInstitutionCourse() {
            this.isSubmitting = true;

            try {
                const response = await axios.post("/api/adminInstitutionCourse", {
                    uniqueId: this.slugValue,
                    courseName: this.fields.course,
                    departmentName: this.fields.department,
                    departmentUniqueId: this.fields.department_slug,
                });
        ...
        bind_data() {
            ...
            this.items = this.course.map((item, index) => {
                return {
                    id: index + 1,
                    course: item.course,
        ...
        edit_row(item) {
            this.$router.push(`/ui/editAdminInstitutionCourse/${item.slug}`);
        },
            ...
            swalWithBootstrapButtons
                ...
                .then(result => {
                    if (result.value) {
                        ...
                        axios
                            .delete("/api/deleteInstitutionCourse/" + id)
                            .then((response) => {
                                this.fetchData();
                            }).catch((error) => {
