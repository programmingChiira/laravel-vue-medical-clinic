                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><span>governance</span></li>
                            </ol>
                        </nav>
                    </div>
                </li>
                    ...
                    <div class="custom-table">
                        <div class="table-header">
                            <div class="d-flex align-items-center flex-column flex-sm-row flex-shrink-0">
                                ...
                                <b-modal id="modalxl" size="xl" title="New school board member" centered no-fade>
                                    <div class="row">
                                        <div id="flFormsGrid" class="col-lg-12 layout-spacing">
                                            <div class="box box-shadow">
                                                <div class="panel-body">
                                                    <b-form @submit.prevent="submitInstitutionGovernance">
                        ...
                        <b-table ref="basic_table" responsive hover :items="items" :fields="columns"
                            ...
                            <template #cell(edit)="row">
                                <div class="position-relative">
                                    <router-link style="color: #2196f3;" :to="`/ui/editAdminInstitutionGovernance/` + row.value">
...
export default {
    metaInfo: { title: 'School board :- Intelligent system' },
    data() {
        return {
            items: [],
            columns: [],
    ...
    created() {
        this.goverNance(this.currentPage);
    },
    ...
    methods: {
        ...
        goverNance(page) {
            axios
                .get('/api/institutionGovernances?page=' + page)
                .then((response) => {
                    this.governance = response.data.data;
                    this.currentPage = response.data.current_page;
                    this.lastPage = response.data.last_page;

                    this.items = this.governance.map((governanceData) => {
                        return {
                            id: governanceData.id,
                            name: `${governanceData.first_name} ${governanceData.last_name}`,
                            email: governanceData.email,
                            phone_no: governanceData.phone_no,
                            edit: governanceData.slug,
        ...
        fetchData(page = 1) {
            axios
                .get(`/api/institutionGovernances?page=${page}`)
                .then((response) => {
                    this.items = response.data.data.map((governanceData) => {
                        return {
                            id: governanceData.id,
                            name: `${governanceData.first_name} ${governanceData.last_name}`,
                            email: governanceData.email,
        ...
        fetchInstitutionGeneralSetting() {
            axios.get('/api/institutionGovernances')
                .then(response => {
        ...
        async submitInstitutionGovernance() {
            this.isSubmitting = true;

            try {
                const response = await axios.post("/api/adminInstitutionGovernance", {
                    uniqueId: this.slugValue, // Use the computed slug for submission
                    userFirstName: this.fields.first_name,
                    userSecondName: this.fields.middle_name,
                    userSurName: this.fields.last_name,
                    userMobileNumber: this.fields.phone_no,
                    userEmail: this.fields.email,
                });
        ...
        bind_data() {
            ...
            this.items = this.governance.map((item, index) => {
                return {
                    id: index + 1,
        ...
        edit_row(item) {
            this.$router.push(`/ui/editAdminInstitutionGovernance/${item.slug}`);
        },
            ...
            swalWithBootstrapButtons
                ...
                .then(result => {
                    if (result.value) {
                        ...
                        axios
                            .delete("/api/deleteInstitutionGovernance/" + id)
                            .then((response) => {
                                this.fetchData();
                            }).catch((error) => {
