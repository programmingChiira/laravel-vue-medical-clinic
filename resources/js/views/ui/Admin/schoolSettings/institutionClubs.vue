                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><span>clubs</span></li>
                            </ol>
                        </nav>
                    </div>
                </li>
                        ...
                        <div class="table-header">
                            <div class="d-flex align-items-center flex-column flex-sm-row flex-shrink-0">
                                <div class="mb-3 mb-sm-0 order-0 order-sm-1 mr-sm-2">
                                    <b-button variant="success" class="ml-2" v-b-modal.institutionClubsModalCenter>
                                ...
                                <b-modal id="institutionClubsModalCenter" title="Create club" centered no-fade>
                                    <b-form @submit.prevent="submitInstitutionClub">
                                        <b-form-row class="mb-4">
                                            <b-form-group label="Club" class="col-md-12">
                                                <b-input type="text" placeholder="Club e.g Red Cross, Debate"
                                                    v-model="fields.club"></b-input>
                                            </b-form-group>
                                        </b-form-row>
                                    ...
                                    <template #modal-footer>
                                        <b-button variant="default" data-dismiss="modal"
                                            @click="$bvModal.hide('institutionClubsModalCenter')"><i class="flaticon-cancel-12"></i>
                                            Close</b-button>
                                    </template>
                                </b-modal>
                            </div>
                        ...
                        <b-table ref="basic_table" responsive hover :items="items" :fields="columns"
                            ...
                            <template #cell(edit)="row">
                                <div class="position-relative">
                                    <router-link style="color: #2196f3;" :to="`/ui/editAdminInstitutionClub/` + row.value">
...
export default {
    metaInfo: { title: 'Institution clubs :- Intelligent system' },
    data() {
        return {
            items: [],
            columns: [],
    ...
    created() {
        this.institutionClub(this.currentPage);
    },
    ...
    methods: {
        ...
        institutionClub(page) {
            axios
                .get('/api/institutionClubs?page=' + page)
                .then((response) => {
                    this.club = response.data.data;
                    this.currentPage = response.data.current_page;
                    this.lastPage = response.data.last_page;

                    this.items = this.club.map((clubData) => {
                        return {
                            id: clubData.id,
                            club: clubData.club,
                            edit: clubData.slug,
        ...
        fetchData(page = 1) {
            axios
                .get(`/api/institutionClubs?page=${page}`)
                .then((response) => {
                    this.items = response.data.data.map((clubData) => {
                        return {
                            id: clubData.id,
                            club: clubData.club,
                            edit: clubData.slug,
        ...
        async submitInstitutionClub() {
            this.isSubmitting = true;

            try {
                const response = await axios.post("/api/adminInstitutionClub", {
        ...
        bind_data() {
            ...
            this.items = this.club.map((item, index) => {
                return {
                    id: index + 1,
                    club: item.club,
                    edit: item.slug,
        ...
        edit_row(item) {
            this.$router.push(`/ui/editAdminInstitutionClub/${item.slug}`);
        },
            ...
            swalWithBootstrapButtons
                ...
                .then(result => {
                    if (result.value) {
                        ...
                        axios
                            .delete("/api/deleteInstitutionClub/" + id)
                            .then((response) => {
                                this.fetchData();
                            }).catch((error) => {
