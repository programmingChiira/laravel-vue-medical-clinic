                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><span>streams</span></li>
                            </ol>
                        </nav>
                    </div>
                </li>
                        ...
                        <div class="table-header">
                            <div class="d-flex align-items-center flex-column flex-sm-row flex-shrink-0">
                                <div class="mb-3 mb-sm-0 order-0 order-sm-1 mr-sm-2">
                                    <b-button variant="success" class="ml-2" v-b-modal.institutionStreamsModalCenter>
                                ...
                                <b-modal id="institutionStreamsModalCenter" title="Create stream" centered no-fade>
                                    <b-form @submit.prevent="submitInstitutionStream">
                                        <b-form-row class="mb-4">
                                            <b-form-group label="Stream" class="col-md-12">
                                                <b-input type="text" placeholder="Stream e.g North, South, West"
                                                    v-model="fields.stream"></b-input>
                                            </b-form-group>
                                        </b-form-row>
                                    ...
                                    <template #modal-footer>
                                        <b-button variant="default" data-dismiss="modal"
                                            @click="$bvModal.hide('institutionStreamsModalCenter')"><i class="flaticon-cancel-12"></i>
                                            Close</b-button>
                                    </template>
                                </b-modal>
                            </div>
                        ...
                        <b-table ref="basic_table" responsive hover :items="items" :fields="columns"
                            ...
                            <template #cell(edit)="row">
                                <div class="position-relative">
                                    <router-link style="color: #2196f3;" :to="`/ui/editAdminInstitutionStream/` + row.value">
...
export default {
    metaInfo: { title: 'Institution streams :- Intelligent system' },
    data() {
        return {
            items: [],
            columns: [],
    ...
    created() {
        this.institutionStream(this.currentPage);
    },
    ...
    methods: {
        ...
        institutionStream(page) {
            axios
                .get('/api/institutionStreams?page=' + page)
                .then((response) => {
                    this.stream = response.data.data;
                    this.currentPage = response.data.current_page;
                    this.lastPage = response.data.last_page;

                    this.items = this.stream.map((streamData) => {
                        return {
                            id: streamData.id,
                            thumb: 'profile-19.jpeg',
                            stream: streamData.stream,
                            edit: streamData.slug,
        ...
        fetchData(page = 1) {
            axios
                .get(`/api/institutionStreams?page=${page}`)
                .then((response) => {
                    this.items = response.data.data.map((streamData) => {
                        return {
                            id: streamData.id,
                            stream: streamData.stream,
                            edit: streamData.slug,
        ...
        async submitInstitutionStream() {
            this.isSubmitting = true;

            try {
                const response = await axios.post("/api/adminInstitutionStream", {
                    uniqueId: this.slugValue,
                    streamName: this.fields.stream,
                });
        ...
        bind_data() {
            ...
            this.items = this.stream.map((item, index) => {
                return {
                    id: index + 1,
                    stream: item.stream,
                    edit: item.slug,
        ...
        edit_row(item) {
            this.$router.push(`/ui/editAdminInstitutionStream/${item.slug}`);
        },
            ...
            swalWithBootstrapButtons
                ...
                .then(result => {
                    if (result.value) {
                        ...
                        axios
                            .delete("/api/deleteInstitutionStream/" + id)
                            .then((response) => {
                                this.fetchData();
                            }).catch((error) => {
